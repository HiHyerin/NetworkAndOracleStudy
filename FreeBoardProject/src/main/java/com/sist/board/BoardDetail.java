...
public class BoardDetail extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 변환코드 => 자바를 실행하고 어떤 파일을 브라우저에 전송할지 여부를 설정
		// 브라우저에서 실행할 수 있는 언어: html(text/html), xml(text/xml), json(text/plain)
		response.setContentType("text/html;charset=UTF-8");// html 안에 한글을 포함하고 있다는 의미
		...
		PrintWriter out = response.getWriter();
	      ...
	      out.println("<tr>");
	      out.println("<td colspan=4 class=text-left valign=top height=200><pre>"+vo.getContent().toString()+"</pre></td>");
	      out.println("</tr>");
