...
public class ChatServer implements Runnable{
	...
	//서버가동
	private ServerSocket ss; // 교환소켓 => 접속시에만 처리
	...
	public ChatServer() {
		try {
			// 서버는 두번 실행하면 안된다 (한번만 실행 가능) => 여러번 돌리고 싶으면 PORT를 변경해서 사용
			/* 
			 	서버
			 		P2P : Client 프로그램 서버를 작동 => 게임
			 */
			ss=new ServerSocket(PORT); // 50명 까지만 접근 => 인트라넷(사내)
			// ss=new ServerSocket(PORT,100); default는 50명 , 이 줄처럼 하면 100명까지
	...
	public static void main(String[] args) {
		...
		new Thread(server).start(); // 서버가 갖고 있는 런을 호출해라
	}
	
	
	
	// 통신 => 사용자 요청 받기, 사용자 요청처리 후 응답 => 통신소켓 => 접속마다 생성해야 된다 => 접속자마다 따로 통신 수행
	class Client extends Thread{
		...
		public Client(Socket s) {
			try {
				...
				// 메모리에 저장 => 저장된 위치로부터 클라이언트가 읽어간다 => 신뢰성이 뛰어나다(TCP)
			}catch(Exception ex) {}
		}
		public void run() {
			try {
